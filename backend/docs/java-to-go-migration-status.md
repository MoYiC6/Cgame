...
| 获取订阅模板 | `GET /api/client/subscribe-message/templates` | ⚠️ |
| 记录订阅 | `POST /api/client/subscribe-message/record` | ⚠️ |
| 订阅状态 | `GET /api/client/subscribe-message/status` | ⚠️ |
| 通知列表（管理端） | `GET /api/admin/notifications` | ⚠️ |
| 创建通知 | `POST /api/admin/notifications` | ⚠️ |
| 更新通知 | `PUT /api/admin/notifications/{id}` | ❌ |
| 删除通知 | `DELETE /api/admin/notifications/{id}` | ⚠️ |
